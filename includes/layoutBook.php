...
// Helper per generare le <option> di una datalist
...
?>
<style>
    .book-card{background:#fff;border-radius:1rem;border:1px solid #e5e7eb;box-shadow:0 1px 3px rgba(0,0,0,.06)}
    .field-label{display:block;font-size:.6875rem;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:#9ca3af;margin-bottom:.25rem}
    .field-value{font-size:.9375rem;color:#1f2937;font-weight:500}
    .form-input,.form-select{width:100%;padding:.55rem .75rem;font-size:.875rem;border:1px solid #e5e7eb;border-radius:.5rem;outline:none;background:#fff;transition:box-shadow .15s,border-color .15s;color:#111827}
    .form-input:focus,.form-select:focus{box-shadow:0 0 0 2px #9ca3af40;border-color:#9ca3af}
    .form-input.error,.form-select.error{border-color:#ef4444;box-shadow:0 0 0 2px #ef444420}
</style>
...
<div class="space-y-6">
    ...
    <?php if ($bookMode === 'view'): ?>
        ...
        <div class="p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            foreach ($fields as [$ico, $lbl, $val, $style]): ?>
            <div>
...
<script>
(function(){
    const isEdit=<?= $bookMode==='edit'?'true':'false' ?>, bookId=<?= (int)($book['id']??0) ?>;
    const btnSave=document.getElementById('btn-save');
    const errBox=document.getElementById('form-error'), errMsg=document.getElementById('form-error-msg');
    const showErr=msg=>{errMsg.textContent=msg;errBox.classList.remove('hidden');errBox.scrollIntoView({behavior:'smooth',block:'nearest'})};
    const hideErr=()=>errBox.classList.add('hidden');
    const val=id=>document.getElementById(id).value;

    // Helper per estrarre ID o Nuovo Nome dalla datalist
    const getHybridVal = (inputId, listId) => {
        const text = val(inputId).trim();
        if (!text) return { id: null, new_name: null };
        const option = Array.from(document.getElementById(listId).options).find(opt => opt.value === text);
        return option ? { id: parseInt(option.dataset.id, 10), new_name: null } : { id: null, new_name: text };
    };
    ...
    btnSave.addEventListener('click',async()=>{
        ...
        const payload={
            title,
            author_id: author.id, new_author_name: author.new_name,
            category_id: category.id, new_category_name: category.new_name,
            publisher_id: publisher.id, new_publisher_name: publisher.new_name,
            isbn:val('f-isbn').trim()||null,price:val('f-price')||0,publish_year:val('f-year')||null
        };
        if(!title) {showErr('Il titolo è obbligatorio.');document.getElementById('f-title').classList.add('error');return;}
        if(!author.id && !author.new_name) {showErr('Seleziona o inserisci un autore.');document.getElementById('f-author').classList.add('error');return;}
        if(!category.id && !category.new_name){showErr('Seleziona o inserisci una categoria.');document.getElementById('f-category').classList.add('error');return;}
        btnSave.disabled=true;
        btnSave.innerHTML='<i class="bi bi-arrow-repeat animate-spin"></i> Salvataggio…';
        try{
            const res=await fetch(isEdit?`../api/books.php?id=${bookId}`:'../api/books.php',{method:isEdit?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
            const data=await res.json();
            if(!res.ok)throw new Error(data.message||'Errore');
            window.location.href='catalogo.php?success=1';
        }catch(e){showErr(e.message);}
        finally{btnSave.disabled=false;btnSave.innerHTML='<i class="bi <?= $cfg['saveIcon'] ?>"></i> <?= addslashes($cfg['saveLabel']) ?>';}
    });
    ['f-title','f-author','f-category'].forEach(id=>{const el=document.getElementById(id);if(el){el.addEventListener('input',()=>el.classList.remove('error'));el.addEventListener('change',()=>el.classList.remove('error'));}});
})();
</script>
<?php endif; ?>
